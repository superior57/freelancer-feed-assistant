  ...
  if (document.location.pathname.match(/^(\/projects\/)([a-z]){2}/)) {
    ...
    axios
      ...
      .then(async (response) => {
        ...
        $Async('app-employer-info fl-list-item', function ($el) {
          $($el[0]).append(`<div id="freelancer-helper-details"><a href="https://www.freelancer.com/u/${owner_info.username}?w=f" target="_blank">${owner_info.public_name}</a></div>`);
        });
      });
  }
